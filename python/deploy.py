...
def recognize_plate_easyocr(img, coords,reader):
    ...
    def data_plat(npm, plat, waktu, log_masuk, cursor, koneksi):
        sql = f"INSERT INTO {log_masuk} (uid, plat, waktu_masuk) VALUES (%s, %s, %s)"
        val = (npm, plat, waktu)
        cursor.execute(sql, val)
        koneksi.commit()
        print(f"Data berhasil ditambahkan ke tabel {log_masuk}")
    
    data_plat(data_serial, nomor_plat, timestamp, "log_masuk", cursor, koneksi)
...
while True:
    data_serial_arduino = ser.readline().decode().strip()  # Membaca data dari Arduino
    # data_serial = ser.getData()
    data_serial = int(data_serial_arduino)
    print(f"data serial = {data_serial}")
