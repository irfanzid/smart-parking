...
<div class="container">
    ...
    <form method="post" action="search_keluar.php">
        <br>
        <div class="col-lg-4">
...
$run = mysqli_query($conn, $sql);
    if (isset($_POST['submit_date'])) {
        ...
        if (mysqli_num_rows($query)>0) {
            echo '<div class="mb-2">';
            echo '<form method="post" action="down_keluar_xls.php">';
            echo '<input type="hidden" name="start_date" value="' . $_POST['start_date'] . '">';
            echo '<input type="hidden" name="end_date" value="' . $_POST['end_date'] . '">';
            echo '<input type="submit" name="down_keluar_xls" class="btn btn-success" value="Export Excel">';
            echo '</form>';
            echo '</div>';
